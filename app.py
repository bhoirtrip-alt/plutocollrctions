...
class Order(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    order_number = db.Column(db.String(30), unique=True, nullable=False)  # Increased from 20 to 30
    ...
    phone = db.Column(db.String(15), nullable=False)  # Phone numbers typically 10-15 digits
...
def check_database_health():
    """Check if all required tables exist in the PostgreSQL database"""
    try:
        ...
        tables_query = text("""
            ...
            AND table_name IN ('user', 'product', 'order', 'order_item', 'product_image')
        """)
        
        result = db.session.execute(tables_query)
        existing_tables = [row[0] for row in result]
        
        required_tables = ['user', 'product', 'order', 'order_item', 'product_image']
        ...
        if missing_tables:
            ...
            print("Database schema is healthy - all required tables exist")
            return True
    ...
    with app.app_context():
        try:
            ...
            if check_database_health():
                print("Database schema is healthy")
            else:
            ...
            print("Please check your PostgreSQL connection and database configuration")
            raise
